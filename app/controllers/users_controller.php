...
class UsersController extends AppController{
    ...
    public function register(){
        $this->set('title_for_layout', 'Vending-QR | User registration');
            ...
            try {
               ...
               */
                  ...
                  //  $this->Mailer->sendEmail('','registration', 'with form', $this->data['User']);
                    ...
                    $u_id = $this->User->id;
/*
                    $message = '<p>Thank you for joining Vendingqr!</p>';
                    $message .= '<p>Get started:</p>';
                    $message .= '<ul>';
                    $message .= '<li><a href="' . FULL_BASE_URL . '/projects/">Projects</a></li>';
                    $message .= '<li><a href="' . FULL_BASE_URL . '/codeqrs/list">Qr codes List</a></li>';
                    $message .= '</ul>';
  ...
  */          
                   ...
                   /* if((isset($redirect) || $redirect != '') && $redirect == '/projects/'){
   ...
   /* public function dashboard(){
    ...
    public function login(){
        ...
        if($this->Session->check('Sess.prob')){
            ...
            setcookie("PHPSESSID", '258', time()-3600, "/", "vendingqr.com");
            setcookie("CAKEPHP", '258', time()-3600, "/", "vendingqr.com");
            setcookie("PHPSESSID", '258', time()-3600, "/", "beta.vendingqr.com");
            setcookie("CAKEPHP", '258', time()-3600, "/", "beta.vendingqr.com");
        ...
        if(is_null($this->Auth->checkUser())){
         ...
         /*   if (!$user) {
            ...
            if(isset($user_profile) ){
                ...
                if($petvacay_user){
                    $u_id = $this->User->id;
                    $this->Session->write('User.id', $vending_user['User']['id']);
                    $this->Session->write('User.first_name', $vending_user['User']['first_name']);
                    $this->Session->write('User.last_name',  $vending_user['User']['last_name']);
                    if($this->Session->check('Redirect.url')){
                        $url = $this->Session->read('Redirect.url');
                        $this->Session->delete('Redirect.url');
                        $this->redirect($url);
    ...
    function settings(){
        ...
        if($this->data){            
            ...
            if(md5($this->data['User']['oldPassword'].'vqr') == $user['User']['password']){
                if($this->data['User']['newPassword'] && $this->data['User']['confirmPassword']){
                    if($this->data['User']['newPassword'] == $this->data['User']['confirmPassword']){
                        $user['User']['password'] = md5($this->data['User']['newPassword'].'vqr');
                        $this->User->save($user);
    ...
    public function password($verification = null){
        ...
        if(empty($this->data)){
            ...
            if($this->data['User']['password'] && $this->data['User']['confirm_password']){
                if($this->data['User']['password'] == $this->data['User']['confirm_password']){
                    $user['User']['password'] = md5($this->data['User']['password'].'vqr');
                    $this->User->save($user);
