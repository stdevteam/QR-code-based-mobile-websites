 ...
 */
	function send($content = null, $template = null, $layout = null) {
		...
		$this->__message = array();
                // Added functionality to write email's content into file in tmp folder
                if (Configure::read('debug') > 0) {
                    if($this->Controller->Session->check('Message.email')){
                        $emailContent = $this->Controller->Session->read('Message.email');
                        $path = CAKE_CORE_INCLUDE_PATH.DS."app".DS."tmp".DS."emails".DS;
                        $fileName = $this->template."_".date("Y_n_j_G_i_s").".email";
                        if(file_put_contents($path.$fileName, $emailContent)){
                            $this->Controller->Session->delete('Message.email');
                        }
                    }
                }
		return $sent;
